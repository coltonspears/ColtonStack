...
/// <summary>
/// The Profile settings section: display name and avatar color, loaded from and saved to the
/// server (a real round-trip through the resilience pipeline, persisted with a Dapper.Contrib
/// UPDATE). On save it publishes <see cref="ProfileUpdatedMessage"/> — whoever cares (the people
/// pane) listens; this class doesn't know or mind.
...
/// </summary>
public sealed partial class ProfileSettingsViewModel(
    IColtonStackApiClient api,
    IMessenger messenger,
    IValidator<UpdateProfileRequest> validator,
    ILogger<ProfileSettingsViewModel> logger) : ObservableObject
{
    ...
    [ObservableProperty]
    public partial string SavedText { get; set; } = string.Empty;
    ...
    private async Task SaveAsync(CancellationToken cancellationToken)
    {
        ...
        try
        {
            var updated = await api.UpdateProfileAsync(request.DisplayName, request.AvatarColor, cancellationToken);
